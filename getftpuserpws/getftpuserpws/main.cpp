...
void GetFtp(char* pData,DWORD dwDestIp) {
	...
	if (stricmp(pData,"USER")==0) {
		scanf(pData + 4, "%*[]%s", szUserName);
	}
	else if(stricmp(pData,"PASS")==0)
	{
		scanf(pData + 4, "%*[]%s", szPassword);
...
void DecodeIPPacketf(char* pData) {
	IPHeader* pIPHdr = (IPHeader*)pData;
	int nHeaderLen = (pIPHdr->iphVerLen & 0xf) * sizeof(ULONG);
	switch (pIPHdr->ipProtocol)
	{
	case IPPROTO_TCP: {
		TCPHeader* pTCPHer = (TCPHeader*)(pData + nHeaderLen);
		switch (::ntohs(pTCPHer->destinationPort))
		{
		case 21://ftp
		{
			GetFtp((char*)pTCPHer + sizeof(TCPHeader), pIPHdr->ipDestination);
		}
		break;
		case 80://http
			printf("80");
